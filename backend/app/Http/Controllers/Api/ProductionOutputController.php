...
class ProductionOutputController extends Controller
{
    ...
    private function getOriginalCreator($module, $recordId)
    {
        $firstLog = AuditLog::where('module', $module)
    ...
    public function store(Request $request)
    {
        ...
        // Step 3: Get the real user ID to use later
        $auditCreatorID = $user ? $user->id : null;
        ...
        try {
            foreach ($request->products as $prod) {
                ...
                $production = \App\Models\ProductionOutput::create([
                ...
                \App\Models\AuditLog::create([
                    ...
                    'action' => 'Created',
                    'status' => 'Produced',
                    'created_by' => $auditCreatorID,
                    'performed_by' => $auditCreatorID ?? Unknown,
                ]);
    ...
    private function checkItemStock($item, string $type)
    {
        ...
        if ($priority) {
            ...
            // Delete notification when stock is back to normal (resolved)
    ...
    private function createOrUpdateNotification($item, string $type, string $priority, float $quantity, float $lowStockAlert)
    {
        ...
        // Update existing notification or create new one (prevents duplicates)
    ...
    {
        // Delete all notifications for this item (resolved)
