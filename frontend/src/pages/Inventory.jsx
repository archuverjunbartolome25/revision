...
const handleAddItem = async () => {
  // Basic validation
  if (!newItem.item || !newItem.unit || (inventoryType === "raw" && (!newItem.quantity_pieces || Number(newItem.quantity_pieces) <= 0))) {
  ...
  try {
    ...
    setInventoryData(prev => {
      ...
      if (method === "put") {
        return prev.map(item => item.id === addedItem.id ? addedItem : item);
      }
      // If adding, append
 ...
 const handleOpenAlertModal = (item) => {
  ...
  const handleSaveAlert = async () => {
    ...
    try {
      ...
      setInventoryData(prev =>
        prev.map(i =>
          i.id === selectedItem.id ? { ...i, low_stock_alert: Number(newAlertQty) } : i
        )
      );
        ...
        >
          ...
          {inventoryType === "normal" ? (
            ...
            <>
              ...
              {/* Quantity (Pieces) */}
              <td
                className={getQuantityColor(item.quantity_pieces, item.item)}
                style={{ fontWeight: "bold" }}
              >
  ...
  ReactDOM.createPortal(
    <div className="custom-modal-backdrop" onClick={() => setShowBOMModal(false)}>
      ...
      >
        ...
        <hr/>
        <div className="d-flex justify-content-between mt-2">
        <strong>Product: {finishedGoodsDisplayNames[selectedFinishedGood?.item] || selectedFinishedGood?.item}</strong>
        <button
...
<tbody>
  ...
  ReactDOM.createPortal(
    <div className="custom-modal-backdrop" onClick={() => setShowAddMaterialsModal(false)}>
      <div
        className="custom-modal bg-white rounded shadow"
        style={{ width: "700px" }}
        onClick={(e) => e.stopPropagation()}
      >
        <div className="modal-header">
          <h5><strong>Add / Edit Materials Needed</strong></h5>
          <button
            type="button"
            className="btn-close"
            ...
            <div key={row.id} className="d-flex gap-2 mb-2">
              <select
                ...
                onChange={(e) =>
                  setMaterialRows(prev =>
                    prev.map(r => r.id === row.id ? { ...r, selected: e.target.value } : r)
                  )
                }
              >
                <option value="">-- Select Material --</option>
                {options.map(opt => (
                  <option key={`${opt.value}-${opt.supplier}`} value={opt.value}>
                    {opt.label}
                  </option>
                ))}
              </select>
              ...
              {materialRows.length > 1 && (
                <button
                  ...
                  onClick={() =>
                    setMaterialRows(prev => prev.filter(r => r.id !== row.id))
                  }
                >
                  <FaTrashAlt/>
                </button>
        ...
        <div className="d-flex gap-2 mt-2">
          <button
            ...
            onClick={() => setMaterialRows(prev => [...prev, { id: Date.now(), selected: "" }])}
          >
            + Add More
          </button>
...
<button
  ...
  onClick={async () => {
    const selectedMaterials = materialRows.map(r => r.selected).filter(Boolean);

    // 🛑 Basic validation
    if (!selectedFinishedGood || selectedMaterials.length === 0) {
    ...
    try {
      ...
      const newBOMItems = selectedMaterials.map(rmName => {
        const rawMat = rawInventoryData.find(i => i.item === rmName);
        return {
...
>
  ...
  <div className="mt-2">
    <h6><strong>Current Materials Needed</strong></h6>
    <table className="custom-table">
      <thead>
        <tr>
          <th>Raw Material</th>
      ...
      <tbody>
        {selectedFinishedGood.materials_needed.map((rmName, idx) => {
          const rawMat = rawInventoryData.find(rm => rm.item === rmName);
          return (
            <tr key={idx}>
              <td>{rawMaterialsDisplayNames[rmName] || rmName}</td>
              <td>{rawMat?.supplier_name}</td>
              <td>{rawMat?.unit || "pcs"}</td>
              <td>{rawMat?.quantity}</td>
              <td>{rawMat?.quantity_pieces}</td>
              <td>{rawMat ? Number(rawMat.quantity_pieces || 0) : 0}</td>
              <td>
                <button
                  className="btn btn-sm btn-danger"
                  onClick={async () => {
                    try {
                      const updatedMaterials = selectedFinishedGood.materials_needed.filter(
                        m => m !== rmName
                      );
                      ...
                      showMessage("✅ Material removed successfully!");
                    } catch (err) {
                      console.error(err);
                      showMessage("❌ Failed to remove material");
                    }
                  }}
                >
                 <FaTrashAlt/>
...
{showDeleteConfirm &&
  ReactDOM.createPortal(
    <div className="custom-modal-backdrop" onClick={() => setShowDeleteConfirm(false)}>
      <div
        className="custom-modal bg-white rounded shadow"
        style={{ width: "350px" }}
        onClick={(e) => e.stopPropagation()}
      >
        ...
        <p className="mt-2">
          ...
          <strong> {finishedGoodsDisplayNames[selectedFinishedGood?.item]}</strong>?
        </p>
          ...
          <button
            ...
            onClick={async () => {
              try {
                ...
                setInventoryData(prev =>
                  prev.map(i =>
                    i.id === selectedFinishedGood.id ? { ...i, unit_cost: newPrice } : i
                  )
                );
  ...
  ReactDOM.createPortal(
    <div className="custom-modal-backdrop" onClick={() => setShowAddModal(false)}>
      ...
      >
        ...
        {inventoryType === "normal" && (
          <div className="mb-2">
            <label><strong>Pieces per Unit</strong></label>
            <input
              type="number"
              className="form-control"
              placeholder="e.g., 24 means 1 unit = 24 pcs"
              value={newItem.pcs_per_unit || ""}
              onChange={e => setNewItem(prev => ({ ...prev, pcs_per_unit: e.target.value }))}
            />
          </div>
        )}
        ...
        {inventoryType === "normal" && (
          <div className="mb-2">
            <label><strong>Quantity (Pieces)</strong></label>
            <input
              type="number"
              className="form-control"
              placeholder="Quantity (Pieces)"
              value={newItem.quantity_pcs || ""}
              onChange={(e) => setNewItem({ ...newItem, quantity_pcs: e.target.value })}
            />
          </div>
        )}
      ...
      {inventoryType === "raw" && (
        <div className="mb-2">
          ...
          <input
            ...
            onChange={(e) => setNewItem({ ...newItem, quantity_pieces: e.target.value })}
          />
        </div>
      )}
