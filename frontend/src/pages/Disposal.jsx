...
const rawMaterials = rmRes.data.map(item => ({
  ...
  name: item.item,
  type: "Raw Materials",
  supplier: item.supplier_name || (item.supplier_offers?.[0]?.supplier?.name) || "—",
}));
...
const handleSaveDisposal = async () => {
  try {
    ...
    const disposalDate = newDisposal.disposal_date || new Date().toISOString().split("T")[0];
...
const handleShowDetails = async (disposalId) => {
  try {
    ...
    const res = await axios.get(`http://localhost:8000/api/disposals/${disposalId}`);
      ...
      <div className="main-content">
        ...
        <div className="d-flex justify-content-between align-items-center mb-3 mt-3 flex-wrap">
          ...
          <div className="ms-auto d-flex gap-2">
            ...
            >
              + Dispose {newDisposal.selectedType === "Raw Materials" ? "Raw Materials" : "Finished Goods"}
            </button>
            <button
          ...
          <div className="custom-modal-backdrop">
            <div className="custom-modal" style={{width:"450px"}}>
              <div className="modal-header">
                <h5><strong>For Disposal</strong></h5>
                <button type="button" className="btn-close" onClick={() => setShowAddModal(false)}></button>
              </div>
              <hr />
              <div className="mb-2">
                <label className="form-label"><strong>Disposal Date:</strong></label>
                <input
                  type="date"
                  className="form-control"
              ...
              {/* Items Table */}
              <label className="form-label"><strong>Quantities:</strong></label>
...
>
  ...
  {inventoryItems
    .filter(inv => inv.type === newDisposal.selectedType)
    .map(inv => (
        ...
        {inv.name} ({inv.supplier})
                        ...
                        <td>
                          <input
                            ...
                            onChange={(e) => {
                              const updated = newDisposal.items.map(item =>
                                item.id === row.id ? { ...item, qty: e.target.value } : item
                              );
                              setNewDisposal({ ...newDisposal, items: updated });
                            }}
                          />
                        ...
                        <td>
                          <button className="btn btn-danger btn-sm" onClick={() => removeItem(row.id)}>✕</button>
                        </td>
                      </tr>
                    ))}
              ...
              <div className="d-flex justify-content-between">
                <button className="btn btn-success mb-1" onClick={addNewItem}>+</button>
                <button className="btn btn-primary btn-sm mb-1" onClick={handleSaveDisposal}>Save</button>
              </div>
            </div>
          </div>
        )}
...
{showDetailsModal && selectedDisposal && (
  <div className="custom-modal-backdrop">
    <div className="custom-modal" style={{width:"500px"}}>
      <div className="modal-header">
        <h5><strong>Disposal Details</strong></h5>
        <button
          type="button"
          className="btn-close"
...
<hr/>
<h6 className="mt-3"><strong>Items for Disposal:</strong></h6>
<hr/>
{loadingDetails ? (
  <p>Loading...</p>
) : (
  <table className="table table-striped table-sm">
    ...
    <tbody>
      <tr>
        ...
        <td>{selectedDisposal.quantity}</td>
      </tr>
    </tbody>
  </table>
)}
