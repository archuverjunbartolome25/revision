...
const filteredData = inventoryData
  ...
  .sort((a, b) => {
    if (selectedType === "Finished Goods") {
      ...
      const keyA = a.supplier_name
        ? `${a.item} ${a.supplier_name}`
        : a.item;
      const keyB = b.supplier_name
        ? `${b.item} ${b.supplier_name}`
        : b.item;
  ...
  {loading ? (
    Array.from({ length: itemsPerPage }).map((_, idx) => (
      <tr key={idx}>
        <td><Skeleton /></td>
        <td><Skeleton /></td>
        {selectedType !== "Finished Goods" && <td><Skeleton /></td>}
        <td><Skeleton /></td>
        <td><Skeleton /></td>
        <td><Skeleton /></td>
        <td><Skeleton /></td>
    ...
    currentItems.map((item, index) => (
      <tr key={index}>
        <td>{item.type}</td>
        <td>{item.item}</td>
        {selectedType !== "Finished Goods" && <td>{item.supplier_name || "—"}</td>}
        <td>{item.unit}</td>
        <td>{item.unit_cost ? `₱${Number(item.unit_cost).toLocaleString()}` : "—"}</td>
        <td>{Number(item.quantity).toLocaleString()}</td>
        <td>{Number(item.quantity_pcs || item.quantity_pieces || 0).toLocaleString()}</td>
    ...
    onClick={async () => {
      try {
        const response = await fetch(
          `http://localhost:8000/api/inventory-report-pdf?type=${encodeURIComponent(selectedType)}`
        );
...
<tbody>
  {currentLogs.length > 0 ? (
    currentLogs.map((log, index) => {
      ...
      return (
        <tr key={index}>
          ...
          <td style={{ color, fontWeight: "bold" }}>{sign}{log.quantity}</td>
    ...
    <tr>
      <td colSpan="9" className="text-center">No activity logs found.</td>
    </tr>
  )}
</tbody>
...
<div className="section-box finished-goods">
  {loading ? <Skeleton /> : <FinishedGoodsChart inventory={inventory} />}
</div>
  </>
)}
...
<div className="section-box raw-materials">
  {loading ? <Skeleton /> : <RawMaterialsChart rawMats={rawMats} />}
</div>
  </>
)}
        </div>
