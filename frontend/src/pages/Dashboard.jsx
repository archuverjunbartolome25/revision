...
import { FaTools, FaBoxOpen, FaUndo, FaTrashAlt, FaShoppingCart, FaClipboardList, FaBoxes, FaChartLine, FaRegUser, FaListUl } from "react-icons/fa";
...
function Dashboard() {
  ...
  const [topSellingProduct, setTopSellingProduct] = useState({ name: "N/A", total: 0 });
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (employeeID) {
        ...
        setUserFullName(`${user.firstname || ""} ${user.lastname || ""}`.trim());
        setEmployeeID(user.employeeID || "");
        setRole(user.role || "");
      }

// ✅ Return To Vendor = Pending only (exclude Approved)
const rtvRes = await axios.get("http://localhost:8000/api/return-to-vendor");
const pendingRtv = (rtvRes.data || []).filter(item => item.status === "Pending");
...
setDisposalCount(pendingDisposals);
      ...
      // Total sales orders
      const salesRes = await axios.get("http://localhost:8000/api/sales-orders/count");
      setSalesOrdersCount(salesRes.data.count || 0);

      // Forecast data
      const forecastRes = await axios.get(`http://localhost:8000/api/forecast?period=${forecastPeriod}`);
      setForecastData({
  ...
  useEffect(() => {
    const now = new Date();
    setCurrentMonth(now.toLocaleString("default", { month: "long" }).toUpperCase());
  }, []);
...
const getQuantityStyle = (moduleName, type) => {
  switch (moduleName) {
    case "Production Output":
      return type === "Finished Goods" ? { sign: "+", color: "green" } : { sign: "-", color: "red" };
    case "Sales Order":
...
const filteredLogs = activityLogs.filter((log) => {
  ...
  const matchesProcess =
    logProcess === "All" ? true : (processMap[log.module] || log.module) === logProcess;
 ...
 <NavLink 
  ...
  >
    ...
    <div className="card-header">
      <div className="activity-card-title"><strong>Activity Logs</strong></div>(Latest)

        {activityLogs[0] && (
      <div className="card-subtext">
        <strong> By: {employees[activityLogs[0].employee_id] || activityLogs[0].employee_id}<br/>{" "}
        {processMap[activityLogs[0].module] || activityLogs[0].module}</strong>
      </div>
    )}  
    </div>
    <div class="vr"></div>
    ...
    <div className="card-value">
      {activityLogs.length} Activities
          ...
          <div className="section-box inventory-summary">
            <div className="section-title">Inventory Summary for Finished Goods</div>
            <div className="summary-cards">
              ...
              <div className="summary-card">
                <span>Low Stock: </span>
                <strong>{inventory.filter(i => i.quantity <= i.low_stock_alert).length}</strong>
              </div>
            </div>
          ...
          <div className="section-box most-selling text-center">
            <div className="section-title">Best Performing SKU<br />(This Month)</div>
            <div className="sales-sub-box">
              {topProducts.length > 0 ? (
                topProducts.map((product, index) => (
                  <div className="sales-sub-item" key={index}>
                    <span className="label">{product.product}</span>
                    <span className="value">{product.total_sales.toLocaleString()} sales</span>
                  </div>
                ))
              ) : (
          ...
          <div className="section-box rawmats-summary">
            <div className="section-title">Inventory Summary for Raw Materials</div>
            <div className="summary-cards">
              <div className="summary-card">
                <span>Total Items: </span>
                <strong>{rawMats.length}</strong>
              </div>
              <div className="summary-card">
                <span>Low Stock: </span>
                <strong>{rawMats.filter(r => r.quantity <= r.low_stock_alert).length}</strong>
              </div>
            </div>
