...
const handleAddOrder = async () => {
  ...
  const quantitiesObj = validProducts.reduce((obj, p) => {
    const fg = finishedGoods.find(f => Number(f.id) === Number(p.product_id));
  ...
  const payload = {
    ...
    status: "Pending",
    amount: grandTotal,
    products: validProducts.map(p => ({ product_id: p.product_id, quantity: Number(p.quantity) })),
...
useEffect(() => {
  const total = newOrder.products.reduce((sum, item) => {
    const fg = finishedGoods.find(f => Number(f.id) === Number(item.product_id));
...
<div className="mb-2">
  ...
  {/* Header row */}
  <div className="d-flex align-items-center gap-2 mb-1" style={{ fontWeight: "bold" }}>
    <div style={{ width: "170px" }}>Product</div>
    <div style={{ width: "65px" }}>Quantity(Unit)</div>
    <div style={{ minWidth: "140px", textAlign: "right" }}>Unit | Total</div>
    <div style={{ width: "30px" }}></div> {/* space for remove button */}
  </div>
 ...
 <select
  ...
  onChange={(e) => {
    ...
    const fgSelected = finishedGoods.find(f => f.id === selectedId);
    const updated = [...newOrder.products];
    updated[index] = {
      ...updated[index],
      product_id: selectedId,
      unit_price: fgSelected ? Number(fgSelected.unit_cost) : 0,
      quantity: updated[index].quantity || 0,
    };
    setNewOrder({ ...newOrder, products: updated });

    const total = updated.reduce(
      (sum, item) => sum + (item.unit_price * Number(item.quantity || 0)),
      0
    );
    setGrandTotal(total);
  }}
...
{finishedGoods
  .filter(fg =>
    !newOrder.products.some(prod => Number(prod.product_id) === Number(fg.id)) || Number(fg.id) === Number(p.product_id)
  )
  .map((fg) => (
    <option key={fg.id} value={fg.id}>
      {fg.item}
        ...
        <div style={{ minWidth: "140px", textAlign: "right" }}>
          <strong>
            P{unitPrice.toLocaleString("en-PH", { minimumFractionDigits: 2 })} | 
  ...
  <button
    ...
    onClick={() =>
      setNewOrder({
        ...newOrder,
        products: [...newOrder.products, { product_id: "", quantity: 0 }],
      })
    }
  >
    + Add Product
...
{isModalOpen && selectedOrderIndex !== null && (() => {
  ...
  if (order?.quantities) {
    ...
    } else if (typeof order.quantities === "object" && !Array.isArray(order.quantities)) {
  ...
  return (
    <div className="custom-modal-backdrop">
      <div className="custom-modal" style={{ width: "450px" }}>
        ...
        <hr />
        <p><strong>Sales Order #:</strong> {formatOrderNumber(order)}</p>
        <p><strong>Customer Name:</strong> {customers.find(c => c.id === order.customer_id)?.name || 'Unknown'}</p>
        <p><strong>Location:</strong> {order.location}</p>

        <p><strong>Product Ordered:</strong></p>
        ...
        <p><strong>Date Ordered:</strong> {formatDate(order.date)}</p>
        <p><strong>Expected Delivery Date:</strong> {formatDate(order.delivery_date)}</p>
        <p><strong>Date Delivered: </strong> {order.date_delivered ? formatDate(order.date_delivered) : "Pending"}</p>
        <p><strong>Amount:</strong> ₱{Number(order.amount).toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
