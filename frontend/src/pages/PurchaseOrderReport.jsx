...
useEffect(() => {
  ...
  if (storedEmployeeID && storedEmployeeID !== "undefined" && storedEmployeeID !== "null") {
...
useEffect(() => {
  const fetchPurchaseOrders = async () => {
    setLoading(true);
    try {
      const res = await axios.get("http://localhost:8000/api/purchase-orders");
...
const filteredPurchaseOrders = purchaseOrders.filter(
  (po) => (poStatusFilter === "All" || po.status === poStatusFilter) &&
  (poDateFilter === "" || po.order_date.startsWith(poDateFilter))
);
...
const currentPurchaseOrders = filteredPurchaseOrders.slice(indexOfFirstPO, indexOfLastPO);
const poTotalPages = Math.ceil(filteredPurchaseOrders.length / poItemsPerPage);
...
useEffect(() => {
  ...
  return (
    <div
      className={`dashboard-container ${isSidebarOpen ? "" : "sidebar-collapsed"}`}
      style={{ width: "1397px" }}
    >
...
<div className="d-flex align-items-center gap-2 mb-2">
  ...
  <select
    ...
    onChange={(e) => {
      ...
      setPoCurrentPage(1); // reset pagination
    }}
  >
          ...
          currentPurchaseOrders.map((po) =>
            po.items && po.items.length > 0 ? (
                ...
                <tr key={`${po.id}-${item.id}`}>
                  ...
                  {idx === 0 && (
                    <>
                      ...
                      <td rowSpan={po.items.length}>{po.supplier_name}</td>
              ...
              <tr key={po.id}>
                ...
                <td>{new Date(po.expected_date).toLocaleDateString()}</td>
                <td>{po.status}</td>
...
<div className="text-end mt-3">
  <button
    ...
    onClick={async () => {
      try {
        const url = `http://localhost:8000/api/purchase-order-report-pdf?status=${encodeURIComponent(poStatusFilter || 'All')}${poDateFilter ? `&date=${encodeURIComponent(poDateFilter)}` : ''}`;
        ...
        const cleanStatus = poStatusFilter && poStatusFilter !== 'All'
...
<tbody>
  ...
  switch (moduleName) {
    ...
    default:
      ...
      const { sign, color } = getQuantityStyle(moduleName, log.type);
