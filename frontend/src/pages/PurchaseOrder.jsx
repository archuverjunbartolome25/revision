...
import { FaTools, FaTrash, FaShoppingCart, FaBoxes, FaChartLine, FaRegUser, FaListUl , FaUndo, FaTrashAlt } from "react-icons/fa";
...
const fetchSupplierOffers = async (supplierId) => {
  ...
  const allItemsSelected =
    ...
    items.length >= supplierOffers.length;
...
const handleFormSubmit = async (e) => {
  ...
  try {
    ...
    const rawMaterialNames = ["350ml", "500ml", "1L", "6L", "Cap", "6L Cap", "Stretchfilm", "Shrinkfilm", "Label"];
...
const handleReceiveSave = async () => {
  ...
  try {
    for (let item of orderItems) {
      ...
      if (qty > remainingQty) {
        showMessage(`❌ Cannot receive more than remaining quantity for ${item.item_name}.`);
...
const isAllSelected = currentOrders.every(order => selectedRows.includes(order.id));
const [receivedItems, setReceivedItems] = useState([]);

  const filteredReceivedItems = receivedItems.filter(item => {
  if (filterReceiveDate && item.received_date !== filterReceiveDate) return false;
  return true;
});
...
const currentReceivedItems = filteredReceivedItems.slice(indexOfFirstReceived, indexOfLastReceived);
...
const imageUrl = selectedReceipt?.image_path
  ? `${import.meta.env.VITE_API_BASE_URL || "http://localhost:8000"}/storage/uploads/receipts/${selectedReceipt.image_path}`
  : null;
...
<h2 className="topbar-title">Purchase Requests</h2>
  ...
  <button
    ...
    onClick={() => setFilterDate("")} // Reset date filter
  >
    Show All
  </button>
</div>
...
<div className="topbar-inventory-box mt-3">
  ...
  >
    ...
    <td>{itemDisplayNames[item.item_name] || item.item_name}</td>
...
<div className="d-flex justify-content-between mt-2">
  ...
  <span className="text-muted small align-self-center">
    Page {currentReceivePage} of {Math.ceil(filteredReceivedItems.length / receivedItemsPerPage) || 1}
  </span>
  <button
    className="btn btn-sm btn-light"
  ...
  <div className="custom-modal-backdrop">
    <div className="custom-modal" style={{ width: "450px" }}>
      <div className="modal-header">
        <h5><strong>Purchase Receipt Details</strong></h5>
        <button
          type="button"
          className="btn-close"
      ...
      <hr />
      <p><strong>Purchase Order #:</strong> {selectedReceipt.po_number}</p>
      <p><strong>Supplier:</strong> {selectedReceipt.supplier_name}</p>
      <p><strong>Item Name:</strong> {selectedReceipt.item_name}</p>
      <p><strong>Quantity Received:</strong> {selectedReceipt.quantity_received}</p>
      <p><strong>Date Received:</strong> {formatDate(selectedReceipt.received_date)}</p>

      <hr />
      <h6><strong>Receipt Image</strong></h6>
    ...
    <div className="text-center">
      <div className="mt-2">
        <button
          ...
          onClick={() =>
            window.open(
              `${import.meta.env.VITE_API_BASE_URL || "http://localhost:8000"}/storage/${selectedReceipt.image_path}`,
              "_blank"
            )
          }
        >
    ...
    <div className="d-flex align-items-center">
      ...
      <button
        ...
        onClick={async () => {
          ...
          if (!selectedReceipt?.id) return alert("No receipt selected.");

          const formData = new FormData();
          formData.append("file", receiptFile);
          formData.append("purchase_receipt_id", selectedReceipt.id);
          ...
          try {
            ...
            if (response.data.file_path) {
              ...
              showMessage("⚠️ Upload succeeded but no file returned.");
            }
          } catch (err) {
...
{showReceiptModal && selectedReceipt && (
  <div className="custom-modal-backdrop">
    <div className="custom-modal" style={{ width: "500px" }}>
      <div className="modal-header">
        <h5><strong>Purchase Receipt</strong></h5>
        <button
          type="button"
          className="btn-close"
      ...
      <p><strong>Purchase Order #:</strong> {selectedReceipt.po_number}</p>
      <p><strong>Supplier:</strong> {selectedReceipt.supplier_name}</p>
      <p><strong>Item:</strong> {itemDisplayNames[selectedReceipt.item_name] || selectedReceipt.item_name}</p>
      <p><strong>Quantity Received:</strong> {selectedReceipt.quantity_received}</p>
      <p><strong>Date Received:</strong> {formatDate(selectedReceipt.received_date)}</p>
      ...
      {selectedReceipt.image_base64 ? (
        <div className="text-center mt-3">
          <img
            src={selectedReceipt.image_base64 
...
{isModalOpen && selectedOrder && (
  <div className="custom-modal-backdrop">
    <div className="custom-modal" style={{width:"450px"}}>
      <div className="modal-header">
        <h5><strong>Purchase Order Details</strong></h5>
        <button
          type="button"
          className="btn-close"
      ...
      <hr />
      <p><strong>Purchase Order #:</strong> {selectedOrder.po_number}</p>
...
{receivingModalOpen && (
  <div className="custom-modal-backdrop">
    <div className="custom-modal" style={{width:"500px" }}>
      <div className="modal-header">
        <h5><strong>Receive Items</strong></h5>
        <button
          type="button"
          className="btn-close"
...
<div className="mb-2">
  <label><strong>Expected Delivery Date (Range):</strong></label>
  ...
  return (
    <tr key={index}>
      ...
      <td>
        ...
        {isRollSupplier && item.item_name.toLowerCase().includes("label") && (
...
<div className="d-flex justify-content-between mb-3">
  ...
  title={allItemsSelected ? "All items for this supplier are already added" : ""}
>
  +
</button>
  <div>
  <label><strong>Total Amount:</strong></label>
  <span> {formData.amount != null
