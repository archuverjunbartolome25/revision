...
function ReturnToVendorReport() {
	...
	const filteredReturns = returns.filter((r) => {
		...
		if (reportType === "Monthly") {
			const saleMonth = `${date.getFullYear()}-${String(
				date.getMonth() + 1
			).padStart(2, "0")}`;
	...
	const handleGeneratePDF = async () => {
		try {
			...
			const res = await fetch(
				`http://localhost:8000/api/return-to-vendor/pdf?${query}`
			);
											...
											<tr key={i}>
												...
												{r.items && r.items.length > 0 ? (
													<>
														...
														<td>{formatNumber(r.items[0].quantity)}</td>
													</>
												) : (
													<>
														<td>N/A</td>
