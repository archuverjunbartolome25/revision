...
<div className="custom-modal-backdrop">
  <div className="custom-modal" style={{width:"500px"}}>
    <div className="modal-header">
      <h5><strong>Add Return to Vendor</strong></h5>
    ...
    <div className="mb-2">
      ...
      <input className="form-control" value={newReturn.location || ""} readOnly />
    </div>
    ...
    <div className="mb-2">
      ...
      <input type="date" className="form-control"
        ...
        max={new Date().toISOString().split("T")[0]} 
        onChange={(e) => setNewReturn({ ...newReturn, date_ordered: e.target.value })}
      />
    </div>
    <div className="mb-2">
      ...
      <input type="date" className="form-control"
        ...
        max={new Date().toISOString().split("T")[0]} 
        onChange={(e) => setNewReturn({ ...newReturn, date_returned: e.target.value })}
      />
    </div>
    ...
    {newReturn.products.map((p, index) => (
      <div className="d-flex gap-2 mb-2" key={index}>
        ...
        />
        <button className="btn btn-danger btn-sm" onClick={() => removeProductRow(index)}>X</button>
      </div>
    ))}

    <button className="btn btn-secondary btn-sm mb-2" onClick={addProductRow}>+ Add Product</button>

    <hr />
    <div className="text-end">
      <button className="btn btn-primary btn-sm" onClick={handleAddReturn}>Submit</button>
    </div>
  </div>
</div>
)}
...
<div className="custom-modal-backdrop">
  <div className="custom-modal" style={{width:"500px"}}>
    ...
    <hr />
    <p><strong>Return #: </strong>{selectedReturn.rtv_number}</p>
    <p><strong>Customer: </strong>{customers.find(c=>c.id===selectedReturn.customer_id)?.name || "N/A"}</p>
...
<tbody>
  {selectedReturn.items?.length > 0 ? (
    selectedReturn.items.map((item, idx) => {
      const product = inventoryItems.find(inv => inv.id === item.product_id);
      return (
        <tr key={item.id || idx}>
    ...
    <tr>
      <td colSpan="3" className="text-center text-muted">No products returned</td>
    </tr>
  )}
</tbody>
    ...
    <div className="text-end">
      {selectedReturn.status === "Pending" ? (
        <button className="btn btn-success btn-sm"
          onClick={async () => {
            await axios.put(`http://localhost:8000/api/return-to-vendor/${selectedReturn.id}`, { status: "Approved" });
        ...
        <button className="btn btn-secondary btn-sm" disabled>Approved</button>
      )}
    </div>
  </div>
</div>
