  ...
  <div className="custom-modal-backdrop">
    <div className="custom-modal w-25">
      <div className="modal-header">
        <h5><strong>Add New Supplier</strong></h5>
        <button className="btn btn-close" onClick={() => setShowAddModal(false)}></button>
      </div>
      <hr />
      <div className="mb-3">
  ...
  <div className="custom-modal-backdrop">
    <div className="custom-modal w-50">
      ...
      <div className="mb-3">
        ...
        <p><strong>TIN:</strong> {selectedSupplier?.tin || "—"}</p>
      </div>
      ...
      <div className="d-flex justify-content-between align-items-center mb-2">
        <h6><strong>Raw Materials Offered</strong></h6>
        <button
    ...
    <div className="custom-modal w-25">
      ...
      <div className="mb-3">
        <p>
          ...
          {materialDisplayNames[editingOffer?.material_name] || editingOffer?.material_name}
        </p>
        <label className="form-label"><strong>New Price</strong></label>
        <input
          type="number"
          className="form-control"
          value={editPrice}
      ...
      <div className="text-end mt-3">
        <button
  className="btn btn-primary"
  disabled={savingEdit || !selectedSupplier || !editingOffer}
  onClick={async () => {
    ...
    try {
      ...
      setSupplierOffers((prev) =>
        prev.map((offer) =>
          offer.id === editingOffer.id ? { ...offer, price: editPrice } : offer
        )
      );
...
{materialsLoading ? (
  ...
  <table className="custom-table">
    ...
    <tbody>
      {supplierOffers
        .sort((a, b) => {
          const order = ["350ml", "500ml", "1L", "6L", "Cap", "6L Cap"];
        ...
        .map((mat, i) => (
          <tr key={i}>
            <td>{materialDisplayNames[mat.material_name] || mat.material_name}</td>
            <td>{mat.unit}</td>
            <td>₱ {mat.price}</td>
            <td>
<button
  ...
  onClick={() => {
    if (!mat.rawmat_id) {
      alert("This offer cannot be edited because it has no Raw Material ID.");
      return;
    }
    setEditingOffer({ ...mat, rawmat_id: mat.rawmat_id });
...
>
  ...
  <p className="text-muted">No raw materials listed for this supplier.</p>
)}
  ...
  <div className="custom-modal-backdrop">
    <div className="custom-modal w-25">
      ...
      <div className="mb-3">
        ...
        <div className="mb-2">
          <label className="form-label"><strong>Material Name</strong></label>
<select
