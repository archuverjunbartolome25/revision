...
import { FaRegUser, FaListUl, FaTools, FaUndo, FaTrashAlt } from "react-icons/fa"; // Add these at the top
...
const filteredSalesSummary = salesData.filter((sale) => {
  ...
  if (filterValue) {
    ...
    if (reportType === "Weekly") {
      const saleWeek = `${saleDate.getFullYear()}-W${String(
        Math.ceil(
          ((saleDate - new Date(saleDate.getFullYear(), 0, 1)) / 86400000 + saleDate.getDay() + 1) / 7
        )
      ).padStart(2, "0")}`;
  ...
  // ✅ Filter by Status
  if (statusFilter !== "All") {
    const saleStatus = sale.status || (sale.date_delivered ? "Delivered" : "Pending");
...
const cogsData = React.useMemo(() => {
  return filteredSales.flatMap((sale) => {
    ...
    const quantities = typeof sale.quantities === "string" ? JSON.parse(sale.quantities) : sale.quantities;
...
const currentSalesPage = filteredSalesSummary.slice(salesIndexOfFirstItem, salesIndexOfLastItem);
...
const currentCogsPage = cogsData.slice(cogsIndexOfFirstItem, cogsIndexOfLastItem);
                ...
                <tr key={index}>
                  ...
                  <td>
                    {order.date_delivered
                      ? new Date(order.date_delivered).toLocaleDateString("en-PH", {
                          year: "numeric",
                          month: "short",
                          day: "numeric",
                        })
                  ...
                  <td>
                    ₱
                    {Number(order.total_sales || 0).toLocaleString("en-PH", {
...
<div className="d-flex justify-content-between align-items-center mt-2">
  ...
  <small className="text-muted">
    Page {salesPage} of {Math.ceil(filteredSalesSummary.length / salesItemsPerPage) || 1}
  </small>
  <button
    className="btn btn-sm btn-light"
    disabled={salesPage >= Math.ceil(filteredSalesSummary.length / salesItemsPerPage)}
    onClick={() => setSalesPage(prev => prev + 1)}
  >
    Next →
  </button>
...
<tr
  key={index}
  onClick={() => handleRowClick(filteredSalesSummary.find(s => s.date === sale.date && s.customer_name === sale.customer_name))}
...
>
  ...
  <td>
    ₱{Number(sale.total_sales).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
  </td>
  <td>
    ₱{Number(sale.cogs).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
  </td>
  <td>
    ₱{Number(sale.profit).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
  </td>
        </tr>
      );
    })
...
{showModal && selectedSale && (
  ...
  {selectedSale.products
    ...
    : "N/A"}
      ...
      <table className="table table-bordered text-center" style={{ width: "480px" }}>
        <thead>
          <tr>
            <th>Size</th>
            <th>Cases</th>
        ...
        <tbody>
          {(() => {
            const quantities = selectedSale.quantities ? JSON.parse(selectedSale.quantities) : {};
            const pcsPerCase = { "350ml": 24, "500ml": 24, "1L": 12, "6L": 1 };
            return Object.entries(quantities)
              ...
              .map(([size, qty]) => (
                <tr key={size}>
                  ...
                  <td>{(qty * (pcsPerCase[size] || 1)).toLocaleString()}</td>
                </tr>
              ));
          })()}
        </tbody>
      ...
      <p><strong>Total Sales:</strong> ₱{Number(selectedSale.total_sales).toLocaleString()}</p>
      <p><strong>COGS:</strong> ₱{Number(selectedSale.cogs).toLocaleString()}</p>
      <p><strong>Profit:</strong> ₱{(Number(selectedSale.total_sales) - Number(selectedSale.cogs)).toLocaleString()}</p>
